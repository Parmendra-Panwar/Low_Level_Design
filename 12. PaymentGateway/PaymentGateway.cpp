...
class PaytmGateway : public IPaymentGateway{
    ...
    bool initiatePayment(PaymentRequest* request) override {
        cout << "[Paytm] Initiating payment of " << request->getAmount() << " " << request->getCourncy() << " for " << request->getSender() << ".\n";
...
class RazorPayGateway : public IPaymentGateway{
    ...
    bool initiatePayment(PaymentRequest* request) override {
        cout << "[RazorPay] Initiating payment of " << request->getAmount() << " " << request->getCourncy() << " for " << request->getSender() << ".\n";
...
int main() {
    ...
    cout << "------------------------------\n";
    bool res1 = PaymentController::getInstance().handdlePayment(GatewayType::PAYTM, req1);
    ...
    cout << "------------------------------\n";
    bool res2 = PaymentController::getInstance().handdlePayment(GatewayType::RAZOREPAY, req2);
